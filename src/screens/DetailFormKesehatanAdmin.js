...
export default function DetailFormKesehatanAdmin() {
  ...
  const getLaporan = async () => {
    var config = {
      method: 'get',
      url: 'http://aldo.sutralian.online/api/laporan/screening_by_id/' + id,
      headers: {
  ...
  onDeclined = sts => {
    if (sts == 'process') {
      ...
      var config = {
        method: 'post',
        url: 'http://aldo.sutralian.online/api/laporan/update_screening/' + id,
        headers: {
          Authorization: 'Bearer ' + token,
          'Content-Type': 'application/json',
        },
      ...
      axios(config)
        .then(function (response) {
          ...
          Alert.alert('Berhasil', 'Laporan telah terapprove', [
            {
              text: 'OK',
              onPress: () => navigation.replace('FormScreeningAdmin'),
            },
          ]);
        })
        .catch(function (error) {
  ...
  return (
    <View style={styles.container}>
      <View style={styles.containerHeader}>
        <HeaderBar2 title={'Detail Screening Covid Covid'}></HeaderBar2>
      </View>
      <ScrollView>
        <View style={styles.containerBody}>
          ...
          <Cardlist2
            ...
            nim={data.nim}
            status={data.status}
            gambar={'http://aldo.sutralian.online/api' + data.hasil_test_rapid}
            gambar2={
              'http://aldo.sutralian.online/api' + data.sertifikat_vaksin
            }></Cardlist2>
